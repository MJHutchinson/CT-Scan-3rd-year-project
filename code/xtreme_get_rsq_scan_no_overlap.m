function [Y, Ymin, Ymax] = xtreme_get_rsq_scan_no_overlap(h, A)
...
fseek(fid, (h.data_offset+1)*512, 'bof');
...
for f=1:h.fan_scans:h.scans
    ...
    for g=f:(f+h.skip_scans-1)
        if (g<=h.scans)
            fseek(fid, h.skip_samples*2, 'cof');
            dump = fread(fid, h.samples, 'int16');
            fseek(fid, h.skip_samples*2, 'cof');
            dump = fread(fid, h.samples, 'int16');
    ...
    for g=(f+h.fan_scans-h.skip_scans):(f+h.fan_scans-1)
        if (g<=h.scans)
            fseek(fid, h.skip_samples*2, 'cof');
            dump = fread(fid, h.samples, 'int16');
            fseek(fid, h.skip_samples*2, 'cof');
